...
int read_init_file(const char* filename)
{
	...
	if (f != NULL)
	{
		print_log(stdout, NOTE, __FILE__, __LINE__, "Initializing data from the parameterfile", 1,
		...
		global.JSON = "{}";
		print_log(stdout, ERROR, __FILE__, __LINE__, "Could not open file.", 1,
...
void init_data()
{
	...
	if (n < 0) {
		print_log(stdout, ERROR, __FILE__, __LINE__, "Some error occured!", 0);
		return;
	} else {
	...
	if (r < 1 || global.t[0].type != JSMN_OBJECT) {
		print_log(stdout, ERROR, __FILE__, __LINE__, "JSON object not found!", 0);
		return;
	}
	...
	if ((x = get_token(1, r, "particle")) >= 0) {
		...
		if (global.t[x + 1].type == JSMN_OBJECT) {
			...
			if ((y = get_token(x + 2, object_end, "partile_particle_screening_length")) >= 0)
				...
				// if not defined set default value
				global.partile_particle_screening_length = __PARTICLE_PARTICLE_SCREENING_LENGTH;
		}
	} else {
...
void init_pinningsites_square_lattice()
{
	...
	global.pinning_lattice_constant = lattice_const = global.SX / (double) N_rows;
	...
	print_log(stdout, NOTE, __FILE__, __LINE__, "Square lattice of pinningsites initialized", 4,
		"N_pinningsites = %d\n", k,
		"Rows = %d\n", N_rows,
		"Columns = %d\n", N_columns,
...
void init_pinningsites_triangular_lattice()
{
	...
	// calculate row width
	global.pinning_lattice_constant = lattice_const = global.SX / (double) N_columns;
	...
	global.N_pinningsites = k;
	print_log(stdout, NOTE, __FILE__, __LINE__, "Triangular lattice of pinningsites initialized", 5,
		"N_pinningsites = %d\n", k,
		"Rows: %d\n", N_rows,
		"Columns: %d\n", N_columns,
		"Lattice constant: %.2lf\n", lattice_const,
...
void init_particles_randomly()
{
	...
	{		
		...
		{
			print_log(stdout, ERROR, __FILE__, __LINE__, "Can't place particles randomly", 0);
			exit(1);
		}
	...
	print_log(stdout, NOTE, __FILE__, __LINE__, "Random arrangement of particles initialized", 0);
}
...
void init_particles_square_lattice()
{
	...
	print_log(stdout, NOTE, __FILE__, __LINE__, "Square lattice of particles initialized", 2,
		"N_particles = %d\n", k,
...
void init_particles_triangular_lattice()
{
	...
	global.N_particles = k;
	print_log(stdout, NOTE, __FILE__, __LINE__, "Triangular lattice of particles initialized", 3,
		"N_particles = %d\n", k,
		"Rescaled SY to = %.2lf\n", global.SY,
		"Rows = %d\nColumns = %d\nLattice constant = %.2lf\n", N_rows, N_columns, lattice_const);
}
...
void init_files()
{
	...
	if (global.moviefile == NULL)
	{
		...
		print_log(stdout, ERROR, __FILE__, __LINE__, "Could not create/open movie file", 0);
		exit(2);
	}
	...
	{
		// exit if does not managed to open
		print_log(stdout, ERROR, __FILE__, __LINE__, "Could not create/open statistics file", 0);
		exit(2);
	}
